...
var wt = (() => {
  ...
  Ht = d(ht, "u").replace(/punct/g, q).getRegex(),
  Zt = d(ht, "u").replace(/punct/g, pt).getRegex(),
  dt =
    "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",
  Ut = d(dt, "gu")
    .replace(/notPunctSpace/g, ct)
    .replace(/punctSpace/g, Q)
    .replace(/punct/g, q)
    .getRegex(),
  Wt = d(dt, "gu")
  ...
  Kt = d(Qt, "gu")
    ...
    .replace(/punctSpace/g, Dt)
    .replace(/punct/g, ut)
    .getRegex(),
  Gt = d(/\\(punct)/, "gu")
    .replace(/punct/g, q)
    .getRegex(),
  Vt = d(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/)
  ...
  ft = d(/^!?\[(ref)\](?:\[\])?/)
    .replace("ref", W)
    .getRegex(),
  te = d("reflink|nolink(?!\\()", "g").replace("reflink", gt).replace("nolink", ft).getRegex(),
  ...
  ne = {
    ...D,
    br: d(ot).replace("{2,}", "*").getRegex(),
    text: d(D.text)
      .replace("\\b_", "\\b_| {2,}\\n")
      .replace(/\{2,\}/g, "*")
      .getRegex(),
  },
  P = { normal: J, gfm: Lt, pedantic: Pt },
  $ = { normal: K, gfm: D, breaks: ne, pedantic: ee },
  re = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" },
  tt = (r) => re[r];
function S(r, t) {
  if (t) {
...
`),
            ...
            let k = this.rules.other.nextBulletRegex(f),
              ...
              V = this.rules.other.fencesBeginRegex(f),
              X = this.rules.other.headingBeginRegex(f),
              mt = this.rules.other.htmlBeginRegex(f),
              kt = this.rules.other.blockquoteBeginRegex(f);
            for (; r; ) {
              let N = r.split(
                  `
`,
              ...
              if (
                ((p = N),
                this.options.pedantic
                  ? ((p = p.replace(this.rules.other.listReplaceNesting, "  ")), (A = p))
                  : (A = p.replace(this.rules.other.tabCharGlobal, "    ")),
                V.test(p) || X.test(p) || mt.test(p) || kt.test(p) || k.test(p) || y.test(p))
              )
                break;
              if (A.search(this.rules.other.nonSpaceChar) >= f || !p.trim())
                c +=
                  `
` + A.slice(f);
              else {
...
`)
      ...
      if (n.length === s.length) {
        ...
        for (let i of e)
          a.rows.push(nt(i, a.header.length).map((o, l) => ({ text: o, tokens: this.lexer.inline(o), header: !1, align: a.align[l] })));
        return a;
      }
    }
    lheading(r) {
...
`
  ...
  w = class B {
    ...
    constructor(t) {
      ...
      (this.options.pedantic
        ? ((n.block = P.pedantic), (n.inline = $.pedantic))
        : this.options.gfm && ((n.block = P.gfm), this.options.breaks ? (n.inline = $.breaks) : (n.inline = $.gfm)),
...
` + e.text),
    ...
    inlineTokens(t, n = []) {
      ...
      for (; t; ) {
        ...
        let l;
        if (
          this.options.extensions?.inline?.some((c) => ((l = c.call({ lexer: this }, t, n)) ? ((t = t.substring(l.raw.length)), n.push(l), !0) : !1))
        )
          continue;
          ...
          if (n.hooks) {
            ...
            for (let a in n.hooks) {
              ...
              let i = a,
                o = n.hooks[i],
                l = e[i];
              z.passThroughHooks.has(a)
                ? (e[i] = (u) => {
                    if (this.defaults.async && z.passThroughHooksRespectAsync.has(a))
                      return (async () => {
...
const oe = () => {
    ...
    function o(h) {
      ...
      if ((p.hasAttribute("data-notes") && ((k.notes = p.getAttribute("data-notes")), (k.whitespace = "pre-wrap")), f)) {
        ...
        y
          ? ((k.notes = y.innerHTML), (k.markdown = typeof y.getAttribute("data-markdown") == "string"), (g = null))
          : f.hasAttribute("data-notes") && ((k.notes = f.getAttribute("data-notes")), (k.whitespace = "pre-wrap"), (g = null));
      }
      (g &&
        g.length &&
        ((g = Array.from(g).filter((y) => y.closest(".fragment") === null)),
        (k.notes = g.map((y) => y.innerHTML).join(`
`)),
